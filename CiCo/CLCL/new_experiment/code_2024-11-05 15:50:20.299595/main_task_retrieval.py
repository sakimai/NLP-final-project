...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    #########################################
        ...
        # # Calculate CLIPScore before concatenating/reshaping matrices
        ...
        # visual_features = torch.cat([torch.tensor(v) for v in batch_visual_output_list])
        # text_features = torch.cat([torch.tensor(t) for t in batch_sequence_output_list])
        ...
        print(sim_matrix_i2t[0])
        print(batch_visual_output_list[0].shape)
        print(batch_sequence_output_list[0].shape)
