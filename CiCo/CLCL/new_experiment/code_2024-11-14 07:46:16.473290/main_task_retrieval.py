...
def init_model(args, device, n_gpu, local_rank):
    ...
    print(cache_dir) # prints /home/imai.s/.pytorch_pretrained_bert/distributed
    # print(args.cross_model, model_state_dict, args)
...
def _run_on_single_gpu(model, batch_list_v, batch_list_t, batch_sequence_output_list, batch_visual_output_list,sequence_cls, visual_cls,is_train,hybird=False,alpha=0.5):
    ...
    # Iterate over each item in batch_list_v (visual data) using its index and element
    for idx1, b1 in enumerate(batch_list_v):
        # Assign the current visual mask from the batch to `video_mask`
        video_mask = b1
        # Get the corresponding visual output feature for the current batch item
        visual_output = batch_visual_output_list[idx1]
        # Get the visual class-level output for the current batch item
        visual_output_cls=visual_cls[idx1]

        # Initialize lists to store the similarities for each row (i2t) and each row (t2i)
        each_row = []
        each_row_t2i=[]

        # Iterate over each item in batch_list_t (text data) with its index and element
        for idx2, b2 in enumerate(batch_list_t):
            ...
            # Calculate similarity logits for both image-to-text and text-to-image using the model function
            ...
            # If hybrid mode is enabled, apply an additional similarity transformation
            if hybird==True:
        ...
        # Concatenate all similarities in each_row along the last axis to form a complete row for i2t
        each_row_i2t = np.concatenate(tuple(each_row), axis=-1)
        # Concatenate all similarities in each_row_t2i along the last axis to form a complete row for t2i
        each_row_t2i = np.concatenate(tuple(each_row_t2i), axis=-1)
        # Append the concatenated i2t row similarity to the sim_matrix_i2t
        sim_matrix_i2t.append(each_row_i2t)
        # Append the concatenated t2i row similarity to the sim_matrix_t2i
...
def _run_on_single_gpu_new_mix(model, batch_list_v, batch_list_t, batch_sequence_output_list, batch_visual_output_list,sequence_cls, visual_cls,is_train,hybird=False,alpha=0.5,dual_mix=0.5):
    ...
    with torch.no_grad():
        ...
        for idx1, b1 in enumerate(batch_list_v):
            ...
            # Iterate over each text batch item in batch_list_t, using index `idx2` and item `b2`
            for idx2, b2 in enumerate(batch_list_t):
                text_mask = b2
                sequence_output = batch_sequence_output_list[idx2]
                # sequence_output_cls = sequence_cls[idx2]

                # Compute similarities between the current text and video pair
                # i2t: similarity from image (video) to text
                # t2i: similarity from text to image (video)
                # _tmp: other outputs that are not used here
                i2t,t2i, *_tmp = model.get_similarity_logits(sequence_output, visual_output, text_mask, video_mask,
                                                                         loose_type=model.loose_type,is_train=True)
                i2t = i2t.cpu().detach().numpy()
                t2i=t2i.cpu().detach().numpy()

                # Combine i2t and t2i similarities for this text-video pair using a weighted average controlled by dual_mix
                # dual_mix controls the blend between i2t and t2i similarities
                each_row.append(i2t*dual_mix+t2i*(1-dual_mix))
                each_row_t2i.append(i2t*dual_mix+t2i*(1-dual_mix))

            # Concatenate all similarities for this video across all text pairs, creating a complete row
            each_row_i2t = np.concatenate(tuple(each_row), axis=-1)
            each_row_t2i = np.concatenate(tuple(each_row_t2i), axis=-1)

            # Append the complete similarity rows to the final similarity matrices
            sim_matrix_i2t.append(each_row_i2t)
            sim_matrix_t2i.append(each_row_t2i)
...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    model.eval()
    with torch.no_grad():
        # store masks for text and visual (video) inputs
        batch_list_t = []
        batch_list_v = []
        # store sequence (text) and visual output features
        batch_sequence_output_list, batch_visual_output_list = [], []
        # store class-level sequence and visual output features
        batch_sequence_output_list_cls, batch_visual_output_list_cls, = [], []

        total_text_num = 0
        original_text_ids = []  # List to store original text IDs
        original_video_ids = [] 
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            # Store original IDs
            original_text_ids.extend(input_ids.cpu().numpy())  # Assuming input_ids contains the original text IDs
            original_video_ids.extend(video.cpu().numpy())  # Assuming video contains the original video IDs
            ...
            if multi_sentence_:
                ...
                # identify relevant text segments for this batch
                filter_inds = [itm - s_ for itm in cut_off_points_ if itm >= s_ and itm < e_]

                # if there are valid filter_inds
                if len(filter_inds) > 0:
                    # filtered to include only those indices
                    input_ids, segment_ids, input_mask = input_ids[filter_inds, ...], segment_ids[filter_inds, ...], segment_ids[input_mask, ...]
                    # retrieves the text (sequence) features
                ...
                # if multi_sentence_ is False, jointly processes text and video input to retrieve sequence_output and visual_output
    ...
    #########################################
        ...
        def calculate_clip_score(similarity_matrix, weight=1.0):
            # Apply ReLU-like operation to ignore negative similarities
            similarity_matrix = np.maximum(similarity_matrix, 0)
            # Apply the weighting factor `w` to the positive similarity scores
            clip_score = weight * similarity_matrix
            return clip_score

        # Compute similarity matrices for each pair of video and text data in the mini-batch using a specific model function
        ...
        # converts the lists of similarity matrices into a single matrices along the first axis
        sim_matrix_i2t = np.concatenate(tuple(sim_matrix_i2t), axis=0)
        sim_matrix_t2i = np.concatenate(tuple(sim_matrix_t2i), axis=0)

        # Log initial similarity matrix dimensions before reshaping
        logger.info("before reshape, sim matrix size: {} x {}".format(sim_matrix_i2t.shape[0], sim_matrix_i2t.shape[1]))
        # Calculate an adjusted list of cut-off points for segmenting the batch, adding 1 to each element in cut_off_points_
        cut_off_points2len_ = [itm + 1 for itm in cut_off_points_]
        # Find the maximum length across all segments in the batch; this will be used to pad matrices to a consistent shape
        max_length = max([e_-s_ for s_, e_ in zip([0]+cut_off_points2len_[:-1], cut_off_points2len_)])

        # Initialize lists to hold the padded similarity matrices for both image-to-text and text-to-image comparisons
        sim_matrix_new_i2t = []
        sim_matrix_new_t2i = []

        # Loop through start and end points of each segment in the batch to adjust and pad matrices
        for s_, e_ in zip([0] + cut_off_points2len_[:-1], cut_off_points2len_): 
            # Extract a slice of the similarity matrix for image-to-text between indices s_ and e_
            # Pad this slice with `-inf` values to match `max_length`, effectively ignoring extra padding during calculations
            new_matrix_n=np.concatenate((sim_matrix_i2t[s_:e_],
                                                  np.full((max_length-e_+s_, sim_matrix_i2t.shape[1]), -np.inf)), axis=0)
            # Repeat the above process for the text-to-image similarity matrix
            new_matrix_n_t2i=np.concatenate((sim_matrix_t2i[s_:e_],
                                                  np.full((max_length-e_+s_, sim_matrix_t2i.shape[1]), -np.inf)), axis=0)
            # Append the padded matrices to the new lists for image-to-text and text-to-image
            sim_matrix_new_i2t.append(new_matrix_n)
            sim_matrix_new_t2i.append(new_matrix_n_t2i)

        # Stack the padded matrices along a new dimension to create a batch with consistent dimensions
        sim_matrix_i2t = np.stack(tuple(sim_matrix_new_i2t), axis=0)
        sim_matrix_t2i = np.stack(tuple(sim_matrix_new_t2i), axis=0)
        print(sim_matrix_i2t.shape)
        print(sim_matrix_t2i.shape)

        clip_score_i2t = calculate_clip_score(sim_matrix_i2t, weight=args.dual_mix)
        clip_score_t2i = calculate_clip_score(sim_matrix_t2i, weight=args.dual_mix)

        np.save('clip_score_i2t.npy', clip_score_i2t)
        np.save('clip_score_t2i.npy', clip_score_t2i)

        original_text_ids = np.array(original_text_ids)
        original_video_ids = np.array(original_video_ids)

        # Now you can create a mapping of similarity scores to original IDs
        similarity_results = {
            "text_ids": original_text_ids,
            "video_ids": original_video_ids,
            "clip_score_i2t": clip_score_i2t,
            "clip_score_t2i": clip_score_t2i
        }

        # Save or return the similarity results as needed
        np.save(os.path.join(args.output_dir, "similarity_results.npy"), similarity_results)
        print(os.path.join(args.output_dir, "similarity_results.npy"))
...
def main():
    ...
    if DATALOADER_DICT[args.datatype]["test2"] is not None and args.local_rank == 0:
        test_dataloader, test_length = DATALOADER_DICT[args.datatype]["test"](args, tokenizer)

        # from torch.utils.data import Subset

        # # Create a subset with only the first 3 examples
        # subset_indices = list(range(3))  # Limit to the first 3 indices
        # subset_test_dataset = Subset(full_test_dataloader.dataset, subset_indices)
        # test_dataloader = subset_test_dataset
        # test_length = min(test_length, 3)
