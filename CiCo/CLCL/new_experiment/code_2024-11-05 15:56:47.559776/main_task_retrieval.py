...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    #########################################
        ...
        def calculate_clip_score(visual_embs, text_embs):
            ...
            # Get the score for matched pairs (diagonal elements)
            clip_scores = torch.diagonal(similarity)
            
            return clip_scores.mean().item()

        # Convert embeddings to torch tensors if they aren't already
        ...
        # Calculate CLIPScore
        clip_score = calculate_clip_score(visual_features, text_features)
        np.save(os.path.join(args.output_dir, "clipscore.npy"), clip_score)
        logger.info(f"Average CLIPScore: {clip_score:.4f}")
