...
class ph_DataLoader(Dataset):
    ...
    def _get_text(self, sentance_id):
        ...
        for i, sentance_id in enumerate(choice_sentance_ids):
            ...
            words_index=[0]
            # if words exceeds total_length_with_CLS
            if len(words) > total_length_with_CLS:
                #selected_index = list(np.arange(len(words)-1))
                # uses np.linspace to choose approximately evenly spaced indices, while keeping the CLS token in the beginning
            ...
            # creates an attention mask (input_mask) filled with 1s and segment IDs (segment_ids) with 0s
