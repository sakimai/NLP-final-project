...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    #########################################
        ...
        # Function to calculate CLIPScore-like value
        def calculate_clip_score(visual_embs, text_embs):
            # Reshape to (batch_size * num_tokens, feature_dim)
            visual_embs = visual_embs.view(-1, visual_embs.size(-1))
            text_embs = text_embs.view(-1, text_embs.size(-1))
            ...
            similarity = torch.matmul(visual_embs, text_embs.t())
            
            return similarity

        # Convert embeddings to torch tensors
        visual_features = torch.cat([torch.tensor(v) for v in batch_visual_output_list])  # Shape: [256, 65, 512]
        text_features = torch.cat([torch.tensor(t) for t in batch_sequence_output_list])  # Shape: [256, 32, 512]

        # Calculate CLIPScore
        clipscores = calculate_clip_score(visual_features, text_features)
        print("clip scores")
        print(clipscores.shape)
        clipscores_limited = clipscores[:200, :200].cpu().numpy()
        np.save(os.path.join(args.output_dir, "clipscore_matrix.npy"), clipscores_limited)
        logger.info("CLIPScore matrix calculated and saved.")
        ...
        np.save(os.path.join(args.output_dir, "sim_matrix_i2t.npy"), sim_matrix_i2t)
        np.save(os.path.join(args.output_dir, "sim_matrix_t2i.npy"), sim_matrix_t2i)
