...
def compute_metrics(x):
    # Sort each row in descending order by similarity score (by negating x to sort in ascending order)
    sx = np.sort(-x, axis=1)

    # Extract the diagonal values, which represent the similarity scores of matched pairs
    d = np.diag(-x)
    d = d[:, np.newaxis] # Reshape d to make it compatible for broadcasting in the next step

    # Subtract the diagonal values from each element in the sorted matrix to find the ranks of each pair
    ind = sx - d
    ind = np.where(ind == 0) # Get indices where the rank of a matched pair equals zero
    ind = ind[1] # Select the column indices which give the rank positions

    # Calculate various metrics based on the ranks
    metrics = {}
...
# Function to compute text-to-video retrieval metrics based on a similarity tensor
# Top-k values define which ranks to consider for computing the metrics
