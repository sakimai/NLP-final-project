...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    with torch.no_grad():
        ...
        total_text_num = 0
        original_text_dic = test_dataloader.dataset.sentences_dict  # List to store original text IDs
        original_video_dic = test_dataloader.dataset.video_dict

        data_to_save = {
            "original_text_dic": original_text_dic,
            "original_video_dic": original_video_dic
        }

        with open('ph_test.json', 'w') as json_file:
            json.dump(data_to_save, json_file, indent=4)
        print(original_text_dic)
    ...
    #########################################
        ...
        original_text_dic = np.array(original_text_dic)
        original_video_dic = np.array(original_video_dic)

        # Now you can create a mapping of similarity scores to original IDs
        similarity_results = {
            "text_ids": original_text_dic,
            "video_ids": original_video_dic,
