...
def eval_epoch(args, model, test_dataloader, device, n_gpu,istrain):
    ...
    #########################################
        ...
        # Calculate CLIPScore
        clip_scores = calculate_clip_score(visual_features, text_features)
        np.savez_compressed(os.path.join(args.output_dir, "clipscore_matrix.npz"), clipscore_matrix.cpu().numpy())
